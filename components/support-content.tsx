...
import { Search, HelpCircle, Cpu, User, CreditCard, Gamepad2, Send, Paperclip } from "lucide-react"
...
export default function SupportContent() {
  ...
  const supportCategories = [
    ...
    { id: "technical", icon: <Cpu className="h-5 w-5" />, label: t("technical_issues") },
    { id: "account", icon: <User className="h-5 w-5" />, label: t("account_support") },
  ...
  return (
    <div className="space-y-8">
      <div className="text-center space-y-2">
        <h1 className="text-3xl font-bold tracking-tight text-green-800 dark:text-green-300">{t("support_title")}</h1>
        <p className="text-muted-foreground max-w-2xl mx-auto">{t("support_subtitle")}</p>
      </div>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid gap-2">
                  ...
                  />
                  {formErrors.name && <p className="text-sm text-red-500">{t("required_field")}</p>}
                </div>
                <div className="grid gap-2">
                  ...
                  />
                  {formErrors.email && <p className="text-sm text-red-500">{t("valid_email")}</p>}
                </div>
                <div className="grid gap-2">
                  <Label htmlFor="issueType">{t("issue_type")}</Label>
                  <Select value={contactForm.issueType} onValueChange={handleSelectChange}>
                    <SelectTrigger id="issueType" className={formErrors.issueType ? "border-red-500" : ""}>
                      <SelectValue placeholder={t("issue_type_placeholder")} />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="technical">{t("issue_technical")}</SelectItem>
                      <SelectItem value="account">{t("issue_account")}</SelectItem>
                      <SelectItem value="billing">{t("issue_billing")}</SelectItem>
                      <SelectItem value="gameplay">{t("issue_gameplay")}</SelectItem>
                      <SelectItem value="other">{t("issue_other")}</SelectItem>
                    </SelectContent>
                  </Select>
                  {formErrors.issueType && <p className="text-sm text-red-500">{t("required_field")}</p>}
                </div>
                <div className="grid gap-2">
                  ...
                  />
                  {formErrors.message && <p className="text-sm text-red-500">{t("required_field")}</p>}
                </div>
